...
export class ServerDataService {
    ...
    const pipeline = [
      ...
      },
      {
        $group: {
          ...
          failedRequests: {
            $sum: {
              $cond: [{ $gte: ['$data.responseObject.status', 400] }, 1, 0],
            },
          },
          passedRequests: {
            $sum: {
              $cond: [{ $lt: ['$data.responseObject.status', 400] }, 1, 0],
            },
          },
        },
      },
              ...
              $cond: [
                {
                  $in: [
                    '$data.responseObject.status',
                    [
                      400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 500,
  ...
  findAllByProject(
    ...
    return this.serverdataRepository
      .aggregate([
        {
          $match: matchQuery,
        },
