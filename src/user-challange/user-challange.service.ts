...
export class UserChallangesService {
  async addUserChallange(
    addDeveloperUserChallangeDtos: CreateUserChallangeDto,
  ) {
    await this.userChallangeRepository.deleteMany({
      developer: addDeveloperUserChallangeDtos.developer,
    });
    const userChallange = await this.userChallangeRepository.create({
      ...addDeveloperUserChallangeDtos,
      id: uuid(),
      status: 'submitted',
      rank: 0,
    });
  ...
  async create(createUserChallangeDto: CreateUserChallangeDto) {
    const userChallange = await this.userChallangeRepository.create({
      ...createUserChallangeDto,
      id: uuid(),
      status: 'submitted',
      rank: 0,
    });
    this.developerService.updateUserChallanges(
      userChallange.developer,
      userChallange.id,
    );
  ...
  findByUser(id: string) {
    return this.userChallangeRepository
      .find({ developer: id })
  ...
  findByUserOnly(id: string) {
    return this.userChallangeRepository
      .find({ developer: id })
      .select(['challange', 'developer', 'id', 'proficiency']);
  }

  findByChallange(id: string) {
    return this.userChallangeRepository
      .findOne({ challange: id })
      .populate(populateQuery);
  }

  update(id: string, updateUserChallangeDto: UpdateUserChallangeDto) {
    return this.userChallangeRepository.updateOne(
      { id },
      { ...updateUserChallangeDto },
    );
  }
